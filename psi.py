...
def parse_args(arg: str, optional_separator=" "):
    ...
    for token in arg:
        ...
        print(lexer.depth, token)

        if (
            token == TOKEN_WHITESPACE
            and not lexer.state_against(STATE_LIST)
            and lexer.nposisstr()
            and lexer.state_against(STATE_INIT)
        ):  # there's a space and it isnt in a string
        ...
        elif token == TOKEN_STRING and not lexer.state_against(
            STATE_STRING
        ):  # if its a string
    ...
    if _arg[0] == "help":
        ...
        for i in ext:
            ...
            for cm in mod.ECIMain.cmds:
                if kevinshtein_distence(_arg[0], cm) < 2:
                    found = Almost
                    couldbe = cm
